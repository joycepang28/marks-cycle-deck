...
| 市场 | 指数 | 阶段判断依据 |
|------|------|-------------|
| 🇺🇸 美国 | S&P 500 | TTM PE、CAPE、恐贪指数 |
| 🇸🇬 新加坡 | STI | TTM PE、价格动量 RSI(14) |
| 🇭🇰 香港 | 恒生指数 | TTM PE、VHSI 波动率指数 |
| 🇯🇵 日本 | 日经 225 | TTM PE、NKVI 波动率指数 |
| 🇲🇾 马来西亚 | FBM KLCI | TTM PE |
| 🇨🇳 中国 A 股 | 中证 500 | TTM PE（CSIndex 官方汇总法）|
...
- **定义：** 基于恒指期权隐含波动率，方法论与 VIX 相同，由恒生指数公司发布
- **范围显示：** 10（历史低位/平静）→ 60（历史高位/恐慌）
- **颜色方向：与 FGI 相反** — 左绿（低波动 = 平静）→ 右红（高波动 = 恐慌）
...
- **来源：** Yahoo Finance `^NKVI.OS`，通过 yfinance 获取 `fast_info.last_price`
- **定义：** 基于大阪交易所日经 225 期权隐含波动率
- **范围显示：** 10 → 60（与 VHSI 同标准）
- **颜色方向：与 VHSI 相同**（低波动 = 绿，高波动 = 红）
- **历史参考：** 10年范围约 13–71；正常区间约 15–30
...
- 马来西亚无官方 VIX 类指数
- 中国 A 股暂无稳定可自动抓取的中文情绪指数源（东方财富 f162 字段对指数无效）
...
my:  level, ytd, ttmPE, pe5y, pe10y
cn:  level, ytd, ttmPE, pe5y, pe10y
```
